import { toInt, thousandSeperator, getStrLevel, getStrAmount } from '/warrobots-calculator/js/data-helper.js?v=1.10.0';
...
function syncData() {
    ...
    for (i = 0; i < DS_ULTIMATE_ROBOT_UPGRADES.length; i++) {
        if ((DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 1 && DS_ULTIMATE_ROBOT_UPGRADES[i].level == 1) ||
            (DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 2 && DS_ULTIMATE_ROBOT_UPGRADES[i].level == 1)
        ) {
            continue;
        }
        ...
        silverAmount = applyUpgradeDiscountPercentage(upgradeDiscountPercentage, DS_ULTIMATE_ROBOT_UPGRADES[i].silverAmount) * quantity;
        upgradeTokens = DS_ULTIMATE_ROBOT_UPGRADES[i].upgradeTokens * quantity;
...
function presetUpgrade(type) {
    ...
    for (i = 0; i < DS_ULTIMATE_ROBOT_UPGRADES.length; i++) {
        ...
        if (type == 1) {
            ...
            if (DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 2 &&
                DS_ULTIMATE_ROBOT_UPGRADES[i].level > 0) {
                continue;
            }
            // Skip all MK3.
            else if (DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 3) {
                continue;
            }
        }
        // MK2 to MK3.
            ...
            // Skip all MK1.
            if (DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 1) {
                continue;
            }
            // Skip MK2: Level 0 only.
            else if (DS_ULTIMATE_ROBOT_UPGRADES[i].mark == 2 &&
                DS_ULTIMATE_ROBOT_UPGRADES[i].level == 0) {
                continue;
            }
        }
...
function init() {
    ...
    for (i = 0; i < DS_ULTIMATE_ROBOT_UPGRADES.length; i++) {
        ...
        upgradeContainerInnerHTML += '<div class="col-md-6">' +
            ...
            '<div class="col">' + '<div class="item-title">' + getStrLevel(DS_ULTIMATE_ROBOT_UPGRADES[i].mark, DS_ULTIMATE_ROBOT_UPGRADES[i].level) + '</div>' +
            '</div>' +
            '<div class="col">' +
